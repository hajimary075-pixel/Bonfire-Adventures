...
const Bonfire = (function () {
  ...
  function hotelCardHTML(h) {
    const priceHTML = h.price
      ? formatKES(h.price) + '<br><small>from · ' + h.priceBasis + (h.boardBasis ? ' · ' + h.boardBasis : '') + '</small>'
      : '<span style="font-size:.85rem;">Check Availability</span>';
    const amenityTags = (h.amenities || []).slice(0, 2).map(function (a) { return '<span>' + a + '</span>'; }).join('');
    return (
      '<article class="card card--compact" data-analytics="hotel-card" data-id="' + h.id + '">' +
        '<div class="card-media card-media--sm" style="background-image:url(\'' + h.image + '\')"><span class="badge">' + h.accommodationType + '</span></div>' +
        '<div class="card-body">' +
          '<div class="card-loc">' + h.town + ' · ' + h.region + '</div>' +
          '<h3 class="card-title card-title--sm">' + h.name + '</h3>' +
          '<div class="card-tags">' + amenityTags + '</div>' +
          '<div class="card-foot">' +
            '<div class="price">' + priceHTML + '</div>' +
            '<div class="card-actions">' +
              '<a class="btn btn-ghost btn-sm" href="pages/hotels.html?id=' + h.id + '" data-analytics="hotel-view">Details</a>' +
